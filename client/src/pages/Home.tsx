        ...
        <div className="max-w-7xl mx-auto text-center relative z-10">
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold leading-tight mb-6 max-w-5xl mx-auto">
              Building <span className="text-gradient">Bold Backends</span> & <br className="hidden md:block" />
              Seamless <span className="text-gradient">Shopify Apps</span>
            </h1>

            <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-10 leading-relaxed">
              Hey, I’m <span className="text-foreground font-semibold">Sampann</span> – a Web Developer blending powerful backends with clean UI. I specialize in custom Shopify apps that actually scale. 💻🚀
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Button size="lg" className="h-14 px-8 rounded-full bg-gradient-primary text-lg font-medium shadow-lg shadow-primary/25 hover:shadow-xl hover:shadow-primary/30 transition-all hover:-translate-y-0.5" asChild>
                <a href="#projects">
              ...
              <Button size="lg" variant="outline" className="h-14 px-8 rounded-full text-lg border-2 hover:bg-secondary/50" asChild>
                <a href="#contact">
                  Let's Talk <Sparkles className="ml-2 w-5 h-5 text-yellow-500" />
                </a>
              </Button>
            </div>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              ...
              <p className="text-muted-foreground max-w-xl">
                A selection of applications and tools I've built, focusing on performance, scalability, and user experience.
              </p>
            </div>
          ...
          {isLoading ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-96 rounded-2xl bg-card/30 animate-pulse border border-border/50" />
              ))}
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
